...
# 11. Curly Braces {} - Specifying Exact Occurrences
    ...
    # Explanation: The pattern lo{2} searches for an 'l' followed by exactly two 'o's. In our test sentence, it successfully identifies 'loo' within the words 'loom' and 'balloon', demonstrating the ability of curly braces {} to specify an exact number of occurrences. We can establish a range as well. The pattern lo{2,5} would specify anywhere from 2 to 5 'o's
...
# 19. \B - used to mark something that isn't the start or end of a word
    ...
    # Explanation:\B is used to find matches where a word Boundary does not exist. In this case, it ensures that the pattern 'll' is not at the start or end of a word. The regex pattern \Bll\B specifically looks for occurrences of 'll' where both the preceding and following characters are also word characters (like letters or digits). It ignores cases where 'll' is at the beginning or end of a word.